...
export default class Ban extends Command {
  ...
  public async run(client: Client, message: Message, args: string[]): Promise<any> {
    ...
    if (ifMemberStaff || member.permissions.has(PermissionFlagsBits.Administrator)) return message.reply('anda tidak bisa memalu staff.')

    await member.send(`Anda telah dipalu dari ${message.guild.name} dengan alasan:\n\`\`\`${reason}\`\`\``)
      .catch(_err => {
        // Do fucking nothing
      })
